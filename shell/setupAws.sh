...
echo "Checking if DynamoDB table exists: $DYNAMODB_TABLE_ISSUER"
TABLE_EXISTS=$(aws dynamodb describe-table --table-name "$DYNAMODB_TABLE_ISSUER" --region $AWS_DEFAULT_REGION 2>/dev/null)
